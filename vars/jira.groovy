...
def getIssueByVersion(project, version) {
    ...
    SH_CMD = sh (
        ...
        script: '''
                    set +x  
                    jiracli list --template table --query "project = \'''' + project + '''\' and affectedVersion IN (\'''' + version + '''\')  ORDER BY priority asc, created" --endpoint=''' + env.JIRA_SERVER + ''' | grep -i APDP | awk -F '|' '{ print $2 }' | sed -e 's/^[[:space:]]*//'
                    set -x
                ''',
        returnStdout: true
    ).trim()
